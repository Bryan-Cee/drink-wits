...
import { FaArrowLeft, FaArrowRight, FaForward, FaUndo, FaUsers } from 'react-icons/fa';
  ...
  const _handleFavorite = async () => {
    ...
    if (currentCardIndex < cards.length) {
      ...
      try {
        ...
        toast.success(currentCard.isFavorite ? 'Removed from favorites!' : 'Added to favorites!');
      } catch (_error) {
        ...
        {showPlayers && (
          <div className="absolute top-10 left-0 z-10 bg-white dark:bg-gray-800 rounded-md shadow-md p-3 text-sm border dark:border-gray-700 w-48">
            ...
            <ul className="space-y-1">
              {players.map((player) => (
                <li key={player} className="flex items-center text-gray-800 dark:text-gray-200">
